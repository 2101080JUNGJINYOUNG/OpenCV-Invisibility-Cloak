...
영화 해리포터의 "투명 망토"처럼, 특정 색상의 천을 두르면 그 부분만 화면에서 사라진 것처럼 보이게 만드는 실시간 영상 처리 프로그램입니다. 원리는 간단합니다: 사람이 없는 배경을 미리 한 장 찍어두고, 이후 매 프레임에서 지정한 색(기본값: 빨강)이 검출된 영역만 그 배경 이미지로 바꿔치기합니다. 색상이 검출된 자리에는 저장해둔 배경이, 나머지 자리에는 지금 촬영 중인 실제 모습이 그대로 나오기 때문에, 색깔 천을 두른 부분만 배경과 하나가 되어 "투명해진 것"처럼 보입니다.
