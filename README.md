...
색상 검출부터 배경 합성, UI 상호작용, 영상 저장까지 이 프로젝트를 만들어오면서 가장 크게 느낀 건 "이론으로 아는 것"과 "코드로 직접 다뤄봐야 아는 것" 사이의 간극이었습니다. 처음에는 `cv2.inRange()`로 빨간색 범위 하나만 지정하면 되는 줄 알았는데, 막상 천을 비추자 마스크에 구멍이 숭숭 뚫린 걸 보고서야 빨강이 HSV 색상환의 0° 근처와 180° 근처 양쪽에 걸쳐 있다는 사실을 몸으로 깨달았고, `[0,120,70]~[10,255,255]`와 `[170,120,70]~[180,255,255]` 두 범위를 더해야 온전한 마스크가 나온다는 걸 실수를 통해 배운 게 가장 큰 성취였습니다. 마스크를 만든 직후 화면을 보고 "다 됐다"고 생각했지만, 실제로는 잔주름 같은 잡음이 여기저기 튀어서 망토 가장자리가 지저분하게 깜빡였고, `cv2.morphologyEx(..., MORPH_OPEN)`으로 작은 점을 지우고 `cv2.dilate()`로 경계를 살짝 넓힌 다음, 그래도 남는 작은 덩어리는 `cv2.findContours()`와 `cv2.contourArea() < 2000` 조건으로 걸러내야 비로소 깨끗한 결과가 나온다는 걸 직접 눈으로 확인하고 나서야 "마스크 하나로 끝나는 게 아니구나"를 체감했습니다. 합성 단계에서는 `bitwise_and`로 망토 영역엔 배경을, 나머지엔 현재 프레임을 남긴 뒤 `cv2.add()`로 더하기만 했는데 자연스럽게 합쳐지는 걸 보면서, 두 마스크가 서로 겹치지 않게만 설계하면 복잡한 알파 블렌딩 없이도 비트 연산만으로 합성이 가능하다는 원리를 이해하게 됐습니다. UI 쪽에서는 `setMouseCallback()`으로 버튼 클릭을 처음 구현했을 때 클릭한 버튼이 계속 빨갛게 남아있거나 반대로 아예 반응이 없는 문제를 겪었는데, `clicked_button`과 `clicked_time`이라는 상태를 따로 두고 0.3초가 지나면 자동으로 원래 색으로 되돌리는 로직을 넣고 나서야 이벤트 기반 프로그래밍에서는 "지금 눌렸는가"뿐 아니라 "언제까지 눌린 상태로 보여줄 것인가"까지 직접 관리해야 한다는 걸 깨달았습니다. 영상 저장 부분에서는 `out.write(combined)`와 `out2.write(combined[:, sidebar_width:])`처럼 같은 프레임이라도 배열을 어떻게 슬라이싱하느냐에 따라 UI 포함본과 순수 결과본이 갈린다는 걸 알게 되면서, 시연용 결과물과 실제 처리 결과물을 분리해서 관리하는 감각을 익혔습니다. 전체적으로 돌아보면 이 프로젝트는 OpenCV 함수 몇 개를 외우는 것이 아니라, 색이 어떻게 마스크로 바뀌고 그 마스크가 어떻게 비트 연산을 거쳐 하나의 프레임으로 합쳐지는지를 실패와 재시도를 반복하며 눈과 손으로 확인해 나가는 과정이었습니다.
