...
전체적으로 이 프로젝트는 OpenCV 함수 몇 개를 외우는 게 아니라, 색이 어떻게 마스크로 바뀌고 그 마스크가 비트 연산을 거쳐 하나의 프레임으로 합쳐지는지를 실패와 재시도를 반복하며 확인해 나가는 과정이었습니다. 결과적으로 이전 [`OpenCV-Practice`](https://github.com/2101080JUNGJINYOUNG/OpenCV-Practice)에서 개별적으로 학습했던 `VideoCapture`, `VideoWriter`, 마우스 콜백, 마스크와 비트 연산 등의 개념을 하나의 실시간 프로그램으로 연결해 구현할 수 있었습니다. 단순히 각 함수의 사용법을 아는 수준에서 벗어나, 입력 영상의 문제를 직접 분석하고 필요한 처리 과정을 설계해 하나의 동작하는 결과물로 완성했다는 점에서 가장 큰 성취가 있었습니다.
